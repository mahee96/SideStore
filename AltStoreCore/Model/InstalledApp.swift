...
public class InstalledApp: NSManagedObject, InstalledAppProtocol
{
    ...
    @objc public var hasUpdate: Bool {
        ...
        guard let storeApp = self.storeApp,
              let latestVersion = storeApp.latestSupportedVersion else { return false }
        ...
        if let storeAppV2 = storeApp as? StoreAppV2,
           UserDefaults.standard.isBetaUpdatesEnabled,
           let latestBeta = storeAppV2.releaseTrack?.latestRelease(for: UserDefaults.standard.betaUdpatesTrack),
           let betaSemVer = SemanticVersion(latestBeta.version),
           currentSemVer! < betaSemVer {
            
            // Special handling for SideStore app
            if storeAppV2.bundleIdentifier == Bundle.Info.appbundleIdentifier {
                return latestBeta.revision.map(isRevisionNewer) ?? true
            }
    ...
    private func matches(_ appVersion: AppVersion) -> Bool {
...
public extension InstalledApp
{
    ...
    func update(resignedApp: ALTApplication, channel: ReleaseTrack.CodingKeys? = nil, certificateSerialNumber: String?, storeBuildVersion: String?)
    {
...
public extension InstalledApp
{
    ...
    class func supportedUpdatesFetchRequest() -> NSFetchRequest<InstalledApp>
    {
        ...
        let fetchRequest = InstalledApp.fetchRequest() as NSFetchRequest<InstalledApp>
         ...
         let predicate2Format = [
            // isActive && hasUpdate
            "%K == YES AND %K == YES"
         ].joined(separator: " ")
        
        
        let predicateFormat = ["(", predicate1Format, "OR", predicate2Format, ")"].joined(separator: " ")
//        let predicateFormat = [predicate2Format].joined(separator: " ")
        ...
        // predicate 1
         fetchRequest.predicate = NSPredicate(format: predicateFormat,
                                              ...
                                              #keyPath(InstalledApp.version),
                                              
                                              #keyPath(InstalledApp.storeApp.latestSupportedVersion._buildVersion),
                                              #keyPath(InstalledApp.storeBuildVersion),
                                              ...
                                              #keyPath(InstalledApp.isActive),
                                              #keyPath(InstalledApp.hasUpdate)
         )
