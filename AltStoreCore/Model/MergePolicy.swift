            ...
            case is StoreApp where conflict.conflictingObjects.count == 2:
                ...
                if let previousApp = conflict.conflictingObjects.first(where: { !$0.isInserted }) as? StoreApp
                {
                    ...
                    if let previousAppV2 = previousApp as? StoreAppV2,
                       previousApp.managedObjectContext?.deletedObjects.contains(previousApp) ?? false
                    {
                        self.resolveStoreAppV2Conflict(for: previousAppV2)
                    }
        ...
        for conflict in conflicts
        {
            ...
            case let databaseObject as StoreApp:
                ...
                if let dbAppV2 = databaseObject as? StoreAppV2,
                   let incomingAppV2 = contextApp as? StoreAppV2,
                   dbAppV2.bundleIdentifier == incomingAppV2.bundleIdentifier,
                   dbAppV2.managedObjectContext?.deletedObjects.contains(dbAppV2) ?? false
                {
                    self.resolveStoreAppV2Conflict(for: incomingAppV2)
                }
